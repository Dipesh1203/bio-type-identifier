    ...
    <div>
      ...
      >
        ...
        <div className="flex flex-col items-center py-4">
          <div className={`mb-4 p-3 rounded-full ${dragActive ? 'bg-teal-100' : 'bg-slate-200'}`}>
            <Upload size={28} className={dragActive ? 'text-teal-600' : 'text-slate-500'} />
          </div>
          <p className="text-lg font-medium text-slate-700 mb-2">
            {dragActive ? 'Drop your fingerprint image here' : 'Upload Fingerprint Image'}
          </p>
      ...
      <div className="mt-4 text-sm text-slate-500">
        <p className="mb-1">
          <strong>Note:</strong> For best results, use a clear fingerprint image taken in good lighting.
        </p>
        <div className="flex items-center mt-2">
          <Image size={16} className="mr-2 text-slate-400" />
          <span>Recommended: high-contrast images showing clear ridge patterns</span>
        </div>
      </div>
    </div>
  );
